...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        ...
        with open("../performend_experiments/journal/" + exp + "/results/" + dataset + ".json") as json_file:
            results = json.load(json_file)
            for k in results.keys():
                print("*** Fold", k, dataset, " (journal) ***")
                print("Next Event in Mask (train): ", results[k]["next_event_in_mask_ratio_train"])
                print("Next Event in Mask (val): ", results[k]["next_event_in_mask_ratio_val"])
                print("Next Event in Mask (test): ", results[k]["next_event_in_mask_ratio_test"])
                print()
    ...
    """
    print("NAPM Range 95% CI:", np.mean(napm_diff) - (1.96 * np.std(napm_diff)),
          np.mean(napm_diff) + (1.96 * np.std(napm_diff)))
    print("NAPM Range 90% CI:", np.mean(napm_diff) - (1.645 * np.std(napm_diff)),
          np.mean(napm_diff) + (1.645 * np.std(napm_diff)))
    """
    print("NAPM 95% CI:", st.t.interval(0.95, len(napm_diff)-1, loc=np.mean(napm_diff), scale=st.sem(napm_diff)))
    print()

    for val in napm_diff:
        print(np.around(val, decimals=4))
    print()
