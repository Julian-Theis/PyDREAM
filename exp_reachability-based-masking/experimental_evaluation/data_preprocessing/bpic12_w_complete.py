...
if __name__ == "__main__":
    ...
    k = 5
    log = pd.read_csv("../../data/bpic12_w_complete_raw.csv")
    cases = pd.DataFrame(log["case:concept:name"].unique())

    counts = log['concept:name'].value_counts(normalize=False)
    print(counts)
    #to_remove = ["Schedule intervention", "VERIFIED", "RESOLVED", "INVALID", "DUPLICATE"]
    #log = log[~log["concept:name"].isin(to_remove)]
    log.to_csv("../../data/bpic12_w_complete.csv", index=None)
    ...
    while c < k:
        ...
        if train_event_check and test_event_check:
            c += 1
            train_log.to_csv("../../data/bpic12_w_complete_" + str(c) + "_train.csv", index=None)
            validate_log.to_csv("../../data/bpic12_w_complete_" + str(c) + "_validate.csv", index=None)
            test_log.to_csv("../../data/bpic12_w_complete_" + str(c) + "_test.csv", index=None)
            print("Saved fold", c)