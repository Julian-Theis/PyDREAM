...
if __name__ == "__main__":
    ...
    k = 5
    log = pd.read_csv("../../data/sepsis.csv")
    ...
    log.to_csv("../../data/sepsis.csv", index=None)
    ...
    while c < k:
        ...
        if train_event_check and test_event_check:
            c += 1
            train_log.to_csv("../../data/sepsis_" + str(c) + "_train.csv", index=None)
            validate_log.to_csv("../../data/sepsis_" + str(c) + "_validate.csv", index=None)
            test_log.to_csv("../../data/sepsis_" + str(c) + "_test.csv", index=None)
            print("Saved fold", c)