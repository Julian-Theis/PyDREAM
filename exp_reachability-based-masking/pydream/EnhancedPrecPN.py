...
class EnhancedPN:
    ...
    def __init__(self,
        ...
        for place in self.net.places:
            self.place_list.add(str(place))
        for place in self.initial_marking:
            self.place_list.add(str(place))
        self.place_list = list(self.place_list)
        ...
        self.rg = ReachabilityGraph(net, initial_marking, self.place_list)
        ...
        while log_wrapper.iterator_hasNext():
            ...
            for event in trace:
                if event[self.activity_key] in self.trans_map.keys():
                    ...
                    self.rg.state_to_state_move(old_marking, marking, event[self.activity_key])
    ...
    def explore_reachability_graph(self, log: EventLog):
        variants = variants_filter.get_variants(log)
        variant_log = []
        for v in variants.keys():
            variant_log.append(variants[v][0])

        for trace in tqdm(variant_log, desc="Explore Reachability Graph"):
            places_shortest_path_by_hidden = get_places_shortest_path_by_hidden(self.net, self.MAX_REC_DEPTH)
            marking = copy(self.initial_marking)
            ...
            for event_id in range(len(trace)):
                ...
                ### EXPLORE PATHS OF CURRENT STATE HERE
                for potential in self.trans_map.keys():
                    if potential is None:
                        continue
                    new_marking = old_marking
                    toi = self.trans_map[potential]
                    """ If Transition of interest is not enabled yet, then go through hidden"""
                    if not semantics.is_enabled(toi, self.net, new_marking):
                        _, _, act_trans, _ = apply_hidden_trans(toi, self.net, copy(new_marking),
                                                                places_shortest_path_by_hidden, [], 0, set(),
                                                                [copy(new_marking)])
                        for act_tran in act_trans:
                            new_marking = semantics.execute(act_tran, self.net, new_marking)
                    """ Fire transition of interest """
                    if semantics.is_enabled(toi, self.net, new_marking):
                        new_marking = semantics.execute(toi, self.net, new_marking)
                        self.rg.state_to_state_move(old_marking, new_marking, potential)
                        #print("FOUND POTENTIAL AND ADDED")
                ...
                if event[self.activity_key] in self.trans_map.keys():
                    ...
                    ### EXPLORE PATHS OF REACHED STATE HERE
                    marking_to_investigate = marking
                    for potential in self.trans_map.keys():
                        if potential is None:
                            continue
                        new_marking = marking
                        ...
                        if semantics.is_enabled(toi, self.net, new_marking):
                            new_marking = semantics.execute(toi, self.net, new_marking)
                            self.rg.state_to_state_move(marking_to_investigate, new_marking, potential)
                            #print("FOUND POTENTIAL AND ADDED")

    def decay_replay(self,
                     log_wrapper: LogWrapper,
                     update_rg: bool = False,
        ...
        while log_wrapper.iterator_hasNext():
            ...
            for event_id in range(len(trace)):
                ...
                if event[self.activity_key] in self.trans_map.keys():
                    prefix.append(event[self.activity_key])
                    ...
                    if update_rg and validReachable:
                        self.rg.state_to_state_move(old_marking, marking, event[self.activity_key])
                    ...
                    if time_past is not None:
                        ...
                        if create_masks:
                            masked_events = self.rg.getOutgoingTransitionNamesFromBM(np.array(timedstatesample.getMarkingVec()))
                            timedstatesample.setMask(masked_events)
                            timedstatesample.isNextEventInMask()
                            timedstatesample.setPrefix(copy(prefix))
    ...
    def saveToFile(self, file: str):
        ...
        with open(file + "_rg", "wb") as output:
            pickle.dump(self.rg, output, pickle.HIGHEST_PROTOCOL)
    ...
    def loadFromFile(self, file: str):
        ...
        with open(file + "_rg", "rb") as input:
            self.rg = pickle.load(input)
