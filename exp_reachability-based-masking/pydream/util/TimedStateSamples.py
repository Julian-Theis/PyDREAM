...
class TimedStateSample:
                ...
                if str(place) in [str(key) for key in marking.keys()]:
                    for key, val in marking.items():
                        if str(key) == str(place):
                            marking_vector.append(float(val))
                            break
                else:
                    marking_vector.append(0.0)

            #marking_vector = [0]*(len(self.place_list))
            #for p in marking:
            #    marking_vector[self.place_list.index(str(p))] = 1
    ...
    def getNextEvent(self):
        return self.data["nextEvent"]

    def setMask(self, mask):
        self.data["mask"] = mask

    def setCaseIdentifier(self, caseid):
        self.data["caseid"] = caseid

    def getMarkingVec(self):
        return self.data["TimedStateSample"][2]

    def isNextEventInMask(self):
        isIn = (self.data["nextEvent"] in self.data["mask"])
        self.data["nextEventInMask"] = isIn
