...
class NAPM:
    def __init__(self,
        ...
        if tss_train_file is not None and tss_val_file is not None and tss_test_file is not None:
            ...
            x = Dense(tss_size, activation=self.opts["activation_function"],
                      #kernel_regularizer=regularizers.l1_l2(l1=1e-5, l2=1e-4),
                      #bias_regularizer=regularizers.l2(1e-4),
                      #activity_regularizer=regularizers.l2(1e-5)
                      )(input_tss)
            x = Dropout((self.opts["dropout_rate"]))(x)
            x1 = Dense(mask_size, activation=self.opts["activation_function"],
                       #kernel_regularizer=regularizers.l1_l2(l1=1e-5, l2=1e-4),
                       #bias_regularizer=regularizers.l2(1e-4),
                       #activity_regularizer=regularizers.l2(1e-5)
                       )(input_mask)
            ...
            x = Dense(int(tss_size * 1.2), activation=self.opts["activation_function"],
                      #kernel_regularizer=regularizers.l1_l2(l1=1e-5, l2=1e-4),
                      #bias_regularizer=regularizers.l2(1e-4),
                      #activity_regularizer=regularizers.l2(1e-5)
                      )(x)
            x = Dropout((self.opts["dropout_rate"]))(x)
            x = Dense(int(tss_size * 0.6), activation=self.opts["activation_function"],
                      #kernel_regularizer=regularizers.l1_l2(l1=1e-5, l2=1e-4),
                      #bias_regularizer=regularizers.l2(1e-4),
                      #activity_regularizer=regularizers.l2(1e-5)
                      )(x)
            x = Dropout((self.opts["dropout_rate"]))(x)
            x = Dense(int(tss_size * 0.3), activation=self.opts["activation_function"],
                      #kernel_regularizer=regularizers.l1_l2(l1=1e-5, l2=1e-4),
                      #bias_regularizer=regularizers.l2(1e-4),
                      #activity_regularizer=regularizers.l2(1e-5)
                      )(x)
    ...
    def train(self,
        ...
        event_dict_file = str(checkpoint_path) + "/" + str(name) + "_napm_onehotdict.json"
        with open(str(event_dict_file), 'w') as outfile:
            json.dump(self.one_hot_dict, outfile)

        with open(checkpoint_path + "/" + name + "_napm_model.json", 'w') as f:
            f.write(self.model.to_json())

        ckpt_file = str(checkpoint_path) + "/" + str(name) + "_napm_weights.hdf5"
        ...
        joblib.dump(self.stdScaler, str(checkpoint_path) + "/" + str(name) + "_napm_stdScaler.pkl")
        if save_results:
            results_file = str(checkpoint_path) + "/" + str(name) + "_napm_results.json"
    ...
    def loadModel(self,
                  path: str,
                  name: str):
        with open(path + "/" + name + "_napm_model.json", 'r') as f:
            self.model = model_from_json(f.read())
        self.model.load_weights(path + "/" + name + "_napm_weights.hdf5")
        with open(path + "/" + name + "_napm_onehotdict.json", 'r') as f:
            self.one_hot_dict = json.load(f)
        self.stdScaler = joblib.load(path + "/" + name + "_napm_stdScaler.pkl")
